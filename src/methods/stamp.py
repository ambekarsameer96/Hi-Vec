...
from src.methods.mrl_utils_new import MRL_auto_gradients_select_data_loop, MRL_merge, MRL_merge_cosine, MRL_merge_mean, MRL_merge_mean_position, mutual_info_score
...
class STAMP(nn.Module):
    ...
    def update_memory(self, x):
        ...
        if self.num_class == 1000:
            ...
            output_origin = MRL_output_select(output_origin, args)
            ...
            for i in range(1, len(x)):
                ...
                outputs.append(outputs_x_aug.softmax(dim=1))
            ...
            output_norm = MRL_output_select(output_norm, args)
            
            filter_ids_0 = torch.where(output_origin.max(dim=1)[1] == output_norm.max(dim=1)[1])
            outputs.append(output_origin.softmax(dim=1))
            for i in range(1, len(x)):
                x_aug = x[i]
                aug_outputs = self.model(x_aug)
                
                aug_outputs = MRL_output_select(aug_outputs, args)
                
                outputs.append(aug_outputs.softmax(dim=1))
            output = torch.stack(outputs, dim=0)
            output = torch.mean(output, dim=0)
    ...
    def adapt(self):
        ...
        if executed == False:
            self.model, nmd = MRL_auto_gradients_select_data_loop(data, self.model, self.optimizer, args)
            args.nmd = nmd
        ...
        if len(data) > 0:
            ...
            output_1_copy1 = tuple(tensor.clone() for tensor in output_1)
            output_1 = MRL_output_select(output_1, args)
            ...
            output_1 = self.model(data)  
            output_1_copy = tuple(tensor.clone() for tensor in output_1)
