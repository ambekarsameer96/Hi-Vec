...
def train_target(cfg, domain, severity=5, type='eval'):
    ...
    if arch in ['officehome_vit', 'domainnet126_vit']:
        model.model.netC.eval()
        model.model.netB.eval()
    elif arch != 'vit':
        model.fc.eval()
        if hasattr(model.model,'netB'):
            model.model.netB.eval()
    ...
    for epoch in range(cfg.TEST.EPOCH):
        if cfg.SHOT.CLS_PAR > 0:
            if 'vit' not in arch:
                model.encoder.eval()
            elif arch in ['officehome_vit', 'domainnet126_vit']:
                model.model.netF.eval()
            else:
                model.eval()
