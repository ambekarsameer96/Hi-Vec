...
# from src.methods.mrl_utils import MRL_output_select

logger = logging.getLogger(__name__)
device = "cuda" if torch.cuda.is_available() else "cpu"

#import mrl output select
# from src.methods.mrl_utils import MRL_output_select
# # import mrl_utils
...
def get_source_features_labels(model, source_loader, ckpt_dir=None):
    ...
    with torch.no_grad():
        for batch_idx, (inputs, targets) in enumerate(source_loader):
            ...
            # features = MRL_output_select(features, args)
            # logits = MRL_output_select(logits, args)
...
def get_accuracy(model: torch.nn.Module,
    ...
    with torch.no_grad():
        for i, data in enumerate(data_loader):
            ...
            # output = MRL_output_select(output, args)
...
#write new one for MRL model
def split_up_model_MRL(model):
    #let featurizer be all layers except the linear layers 
    base_model = model.base_model
    linear_layers = model.linear_layer
    return base_model, linear_layers
...
def get_args():
    ...
    # parser.add_argument('--cfg', default=None, tydata/gui_node/code/github_code_tta/STAMP/cfgs/Online_TTA_os/cifar10_c_lsun/stamp.yaml /home/iml/sameer.ambekar/data/gui_node/code/github_code_tta/STAMP/cfgs/Online_TTA_os/cifar10_c_lsun
    parser.add_argument('--cfg', type=str, default = './cfgs/Online_TTA_os/cifar10_c_tiny/stamp.yaml')
    ...
    parser.add_argument('--output_dir', default='./output', type=str, help='path to output_dir file')
    ...
    #  grad_scaling
    parser.add_argument('--grad_scaling', default=0.5, type=float)
    #model merge
    parser.add_argument('--model_merge', default='mean', type=str)
    # 0.9 for Cifar

    #nested_model_dimension
    parser.add_argument('--nmd', default=0, type=int)
    #disable backprop for ndm layers true 
    parser.add_argument('--nmd_disable_backprop', default=False, type=bool)
    parser.add_argument('--mi_threshold', default=1.4, type=float)
